...
snake position and potentially new food positions as well*/
  ...
  /*Plan on making an animation where the snake explodes piece by piece. Still need more time to think about it
  as I don't entirely understand how this game works yet so currently this method is just for my own reference.
   */
  public void destroySnake()
  {
    for(int i = size - 1; size > -1; i--)
    {
      board[headPositions.get(i).get(1)][headPositions.get(i).get(0)] = '*';
      if(i + 1 != size && board[headPositions.get(i + 1).get(1)][headPositions.get(i + 1).get(0)] == '*')
      {
        board[headPositions.get(i).get(1)][headPositions.get(i).get(0)] = ' ';
      }
    }
